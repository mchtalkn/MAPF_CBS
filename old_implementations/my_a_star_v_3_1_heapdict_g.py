...
def is_valid_move(my_map,prev ,next,constraints,edge_constraints,agent,timestep):
    x=next[0]
    y=next[1]
    return (not my_is_edge_constrained(agent,prev,next,timestep,edge_constraints)
...
def get_path(node):
    ...
    g=node['g']
    while node:
        #print("loc:",node['location'],"g:",node['g'])
        if node['prev']:
            g=node['prev']['g']
        else:
            g=-1
    ...
    while open_list:
        ...
        for direction in directions:
            ...
            if not is_valid_move(my_map,node['location'],new_loc,constraint_table,edge_constraint_table,agent,node['g']+1):
                continue
            if my_is_constrained(agent,new_loc,node['g']+1,constraint_table):
                #print("-1-",location,new_loc)
                if not my_is_constrained(agent,node['location'],node['g']+1,constraint_table):
                    #print("-2-",prev,location)
                    open_list[location]=(f+1.001,g+1,prev)
                continue
            successor = {
            'prev' : node,
            'location' : new_loc,
